...
*R. Stuart Geiger, Alejandra Gonzalez-Beltran, Robert Haines, James Hetherington, Chris Holdgraf, Heiko Mueller, Martin O'Reilly, Tomas Petricek, Jake VanderPlas* (authors in alphabetical order)
...
3. Defining career trajectories that cater to the diverse goals of members within the organization.
...
The terms *Research Software Engineer* (i.e. RSE; most often used by UK partners) and *Data Scientist* (most often used by USA partners) have slightly different connotations, but we will not dwell on those aspects here (see [roles.md](roles.md) for some more thoughts on this). In the current document, we will mostly use the terms RSE and Data Scientist interchangeably, to denote the broad range of positions that focus on software-intensive and data-intensive research within academia. In practice, we find that most people flexibly operate in both worlds simultaneously.
...
**Celebrate Each Other's Work**. While it may not be possible to convince the broader academic community to treat software as first-class research output, data science organizations should explicitly recognize many forms of scientific output, including tools and software, analytics workflows, or non-standard written communication. This is especially true for projects where there is no "owner", such as major open-source projects. Just because your name isn't "first" doesn't mean you can't make a valuable contribution to science. Creating a culture that celebrates these efforts makes individuals feel that their work is valued.
